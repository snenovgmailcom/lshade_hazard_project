#!/usr/bin/env python3
"""
L-SHADE with population logging for computing ℓ (good-state analysis).

Based on r_lshade.py, adds logging of:
- Population state at each generation
- Archive state
- Pbest indices
- F and CR values sampled
"""
...
class LSHADE_Logged:
    def __init__(
        ...
        seed=None,
        disp=False,
        log_every=1,        # Log every k generations (1 = all)
        log_max_gens=None,  # Stop logging after this many generations
    ):
        self.func = func
        ...
        self.log_max_gens = log_max_gens
    ...
    def _update_archive(self, individual):
        if len(self.archive) < self.archive_capacity:
            ...
            idx = self.rng.integers(0, self.archive_capacity)
    ...
    def _should_log(self):
        ...
        if self.log_max_gens is not None and self.nit >= self.log_max_gens:
            return False
        return (self.nit % self.log_every) == 0

    def _log_generation(self, pop, fit):
        """Log population state for ℓ analysis."""
        NP = len(pop)
        p_num = max(2, int(self.p_best_rate * NP))
        pbest_indices = np.argsort(fit)[:p_num].tolist()
        ...
        log_entry = {
            ...
            'NP': NP,
            'm_t': len(pbest_indices),
            's_t': NP + len(self.archive),  # donor pool size
    ...
    def _lshade_generation(self, pop, fit):
        ...
        for i in range(NP):
            ...
            if mu is None:
                ...
                CR = truncnorm.rvs(
                    (0.0 - mu) / self.sigma_cr,
                    (1.0 - mu) / self.sigma_cr,
                    loc=mu,
                    scale=self.sigma_cr,
                    random_state=self.rng,
                )
