...
# r_conc = 0.1 * sqrt(10) ≈ 0.316
R_CONC=0.316

# Function difficulty groups
easy="f1 f2 f3 f4 f6 f9"
medium="f5 f7 f8 f10 f11 f12 f13 f14 f15 f16 f17 f18 f19 f20"
hard="f21 f22 f23 f24 f25 f26 f27 f28 f29 f30"

get_eps() {
  local f=$1
  for e in $easy; do [[ "$f" == "$e" ]] && echo 10 && return; done
  for m in $medium; do [[ "$f" == "$m" ]] && echo 100 && return; done
  for h in $hard; do [[ "$f" == "$h" ]] && echo 500 && return; done
  echo 100  # default
}

for f in f1 f2 f3 f4 f5 f6 f7 f8 f9 f10 f11 f12 f13 f14 f15 f16 f17 f18 f19 f20 f21 f22 f23 f24 f25 f26 f27 f28 f29 f30; do
